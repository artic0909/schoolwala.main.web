          ...
          <div class="table-responsive text-nowrap">
            <table class="table table-hover table-bordered">
              ...
              <tbody class="table-border-bottom-0">
                ...
                <tr>
                  <td>
                    <strong>{{ ($subjects->currentPage() - 1) * $subjects->perPage() + $loop->iteration }}</strong>
                  </td>
                  ...
                  <td colspan="6">
                    <div class="d-flex justify-content-center">
                      <div class="d-flex justify-content-center align-items-center mt-3">
                        ...
                        @else
                        <a href="{{ $subjects->previousPageUrl() }}" class="btn btn-primary me-2">Prev</a>
                        @endif

                        <form action="" method="GET" class="d-flex align-items-center">
                          <input type="number" name="page" value="{{ $subjects->currentPage() }}" min="1"
                            max="{{ $subjects->lastPage() }}" class="form-control text-center me-1" style="width: 70px;"
                            onchange="this.form.submit()" readonly>

                          <span class="mx-1">/</span>
                          <input type="text" readonly value="{{ $subjects->lastPage() }}"
                            class="form-control text-center ms-1" style="width: 70px;">
                        </form>

                        @if ($subjects->hasMorePages())
                        <a href="{{ $subjects->nextPageUrl() }}" class="btn btn-primary ms-2">Next</a>
                        @else
...
<div class="modal fade" id="backDropModalAddClass" data-bs-backdrop="static" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <form class="modal-content" action="{{ route('admin.admin-subjects.store') }}" method="POST" enctype="multipart/form-data">
      @csrf
      ...
      <div class="modal-body">
        ...
        <div class="row g-2">
          <div class="col mb-3">
            <label for="background_image" class="form-label">Subject Background Image<span class="text-danger">*</span></label>
...
<!-- Edit Subject Modal -->
@foreach ($subjects as $subject)
<div class="modal fade" id="backDropModalEditClass{{ $subject->id }}" data-bs-backdrop="static" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <form class="modal-content" action="{{ route('admin.admin-subjects.update', $subject->id) }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <div class="modal-header">
      ...
      <div class="modal-body">
        ...
        @if($subject->background_image)
        <img src="{{ asset('storage/' . $subject->background_image) }}" alt="Current Image" width="100">
        @endif
...
@foreach ($subjects as $subject)
<div class="modal fade" id="backDropModalDeleteClass{{ $subject->id }}" data-bs-backdrop="static" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
...
<script>
  $(document).ready(function() {
    ...
    $('#resetBtn').on('click', function() {
      window.location.href = "{{ route('admin.admin-subjects') }}";
    });
  });
</script>
